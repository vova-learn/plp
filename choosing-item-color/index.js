const saveItemName = function () {
  ...
  const setNewItem = function ($itemCard, itemName, $button) {
    const newName = $button.find('.name').text();
    const newPrice = $button.find('.cost').text();
    const newPhoto = $button.find('.pht').text();
    ...
    $itemCard.data().item[2].value = newPhoto.trim();
  };
